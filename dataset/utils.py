...
def generate_masks(img, mask, mode, patchify_fn):
    ...
    if mode == "organ":  # our approach both all organs or question organs, depends on the dataloader
        organs = patchify_fn(mask)
        restore_idx = torch.any(organs > 0, dim=2, keepdim=True)
    elif mode == "random":  # classic MAE
        noise = torch.rand(N, L, 1, device=img.device)
        restore_idx = (noise > 0.5).bool()
    elif mode == "threshold":  # our new baseline
        # th = torch.quantile(img, 0.1)
        th = 0
        restore_idx = torch.all(img > th, dim=2, keepdim=True)
    else:
        raise Exception(f"unsupported masking mode: {mode}")

    restore_idx = restore_idx[0] if N == 1 else restore_idx  # compatible with batch size 1
    return restore_idx