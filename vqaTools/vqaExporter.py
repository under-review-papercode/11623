...
class vqaExtractor:
    ...
    def __init__(self, split_files):
        ...
        for split_file in split_files:
            self.dataset += json.load(open(split_file, 'r'))
    ...
    def generateAnswerList(self):
            q_to_a = {}
            is_open = {}
            answer_list = []
            for entry in self.dataset:
                quesId = entry['qid']
                gtAns = self.qa[quesId]['answer']
                gtAns = str(gtAns)
                gtAns = gtAns.replace('\n', ' ')
                gtAns = gtAns.replace('\t', ' ')
                gtAns = gtAns.strip()
                gtAns = gtAns.lower()
                gtAns = gtAns.replace('x ray', 'xray')
                gtAns = self.processPunctuation(gtAns)
                gtAns = self.processDigitArticle(gtAns)
                if gtAns not in answer_list:
                    answer_list.append(gtAns)
                q_to_a[quesId] = answer_list.index(gtAns)
                is_open[quesId] = self.qa[quesId]["answer_type"] == "OPEN"

            return answer_list, q_to_a, is_open
